...
项目最初只用于恢复 AirPlay，现在已经扩展为运行在音箱上的本地配置与播放服务：浏览器可以查看设备状态、恢复 AirPlay、切换 Wi-Fi、控制蓝牙和 EQ，也可以调用原厂 KPlayer 播放音乐 URL、管理场景、播放历史、URL 收藏、网络电台和定时停止。
...
| 播放场景 | 保存媒体、音量、停止定时，并按音箱时间自动启动 | 实验能力，场景与计划重启后保留 |
| 媒体库 | 自动记录播放历史，统一收藏、搜索、播放和管理 URL 与网络电台 | 实验能力，完整 URL 只保存在音箱本机 |
| 网络电台 | 保存和播放直播流，常用电台可加入统一媒体库；次要操作收进“更多” | 实验能力，电台列表及顺序重启后保留 |
...
- 场景、URL/电台收藏和最近 100 条播放历史保存在音箱本机，历史会合并同一完整 URL 并累计播放次数；
- 电台首次缓冲可能需要约 6–12 秒；
- 定时停止范围为 1–60 分钟，在音箱端执行；
- 手动停止或清空队列会取消定时，换歌或切换电台不会取消；
- 配置服务重启后会清除定时器，避免旧任务误停。

只应添加可信媒体地址。API 对外返回 URL 时会去除用户信息、查询参数和片段；场景、收藏和历史中的完整地址只写入音箱权限为 `0600` 的自有配置，浏览器通过条目 ID 播放或入队，不会取回私密参数。
