...
本地媒体正在播放时，可以在“正在播放”区域拖动 0–100 音量滑杆并点击“应用音量”。服务通过原厂 KPlayer `RenderingControl` 下发设置，只有 `GetVolume` 回读与目标值一致时才报告成功。
...
电台收藏写入音箱的自有配置目录，服务重启后仍会保留。直播首次缓冲通常比普通文件更慢，页面最多等待约 12 秒确认播放。
