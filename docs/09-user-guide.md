...
3. 系统音量、静音、灯光、麦克风计划、闹钟和提醒目前不提供写入；本地 KPlayer 播放音量可在播放页或场景中设置；
...
场景把媒体地址、显示标题、本地播放音量、播放后停止和自动启动计划组合保存在音箱本机，最多保存 20 个。换手机或电脑访问同一台音箱时仍可使用，不依赖浏览器缓存。

创建场景：

1. 打开“播放”页面，在“场景模式”中点击“新建场景”；
2. 输入场景名称并选择图标；
3. 填写可直接访问的 HTTP/HTTPS 媒体 URL；
4. 设置 0–100 的本地播放音量；
5. 设置 0–60 分钟“播放后停止”，0 表示持续播放；
6. 如需自动启动，开启开关并选择音箱本机时间和执行星期，可使用“每天”“工作日”“周末”快捷选择；
7. 点击“保存场景”。

自动启动由音箱服务执行，浏览器可以关闭。场景卡片会显示计划摘要、按音箱时区计算的下次执行时间，以及上次自动启动成功或失败的结果。为避免播放器在同一时刻被多个场景争抢，同一启动时间且星期有重叠的启用计划不能同时保存。

服务会在开始播放前持久化当次触发标记，因此服务重启后不会在同一分钟重复启动。服务停止期间错过的时间不会补播，以免音箱恢复后意外出声。自动启动依赖音箱系统时间；若执行时间异常，应先通过“诊断”页面或 SSH 检查设备时钟和时区。

启动场景前，页面会明确预览以下影响：

- 替换当前播放和队列；
- 把 KPlayer 音量调整到场景值并回读；
- 定时大于 0 时重新设置倒计时；定时为 0 时取消已有定时；
- 媒体、音量和定时按顺序执行。若中途失败，页面会提醒检查当前状态，因为失败前的步骤可能已经生效。

场景的完整媒体 URL 使用 `0600` 权限写入 `/mnt/UDISK/sanyin-config/player-scenes.json`。API 和页面只显示删除查询参数、用户信息和片段后的公开来源。编辑已有场景时，媒体 URL 留空即可保留设备中的完整地址，适用于包含签名参数的临时或私有链接。

删除场景会同时删除其自动启动计划，但不会停止当前播放，也不会删除网络电台收藏。关闭自动启动后仍可继续使用卡片上的“启动场景”。
...
媒体确认开始播放后会自动写入“播放历史”。相同完整 URL 不会产生重复行，而是更新到列表顶部并累计播放次数；最多保留最近 100 条。统一收藏同样最多 100 个，可通过以下方式创建：

- 在“播放 URL”表单中点击“收藏 URL”；
- 在播放历史条目中点击“收藏”；
- 在已有网络电台卡片的“更多”菜单中点击“收藏到媒体库”；
- 再次收藏同一完整 URL 会更新标题并移动到收藏顶部。

媒体库支持按标题、类型或公开 URL 搜索，并用“URL”或“电台”标识收藏类型。每条收藏和历史均可直接播放、加入队列、复制公开地址或删除；清空播放历史不会影响收藏、当前播放和队列。删除带私密参数的收藏前页面会二次确认，因为公开地址不能还原这些参数。

完整 URL 只保存在音箱的 `/mnt/UDISK/sanyin-config/media-library.json`，文件权限为 `0600`。页面展示和复制的地址已经删除查询参数、用户信息和片段；“播放”和“入队”只向音箱提交条目 ID，由设备端读取完整地址。
...
本地媒体正在播放时，可以在“正在播放”区域拖动 0–100 音量滑杆，不需要再点击确认按钮。页面会即时显示目标值，每 150 毫秒合并一次拖动输入；设备响应期间仍可继续拖动，服务只保留并继续同步最新目标。播放器状态读取与控制会串行访问原厂 KPlayer，最终仍以 `GetVolume` 回读一致作为成功条件。
...
- 页面空闲时每秒回读一次当前值；拖动和设备同步期间暂停页面轮询，避免旧值覆盖目标值，其他原厂控制路径改变音量后仍会更新显示。
...
3. 点击“收藏电台”；
4. 在电台卡片中点击“播放”，或打开“更多”执行其他操作。

电台卡片默认只显示“播放”和“更多”。“收藏到媒体库”、加入队列、上移、下移和删除均在“更多”菜单中；已加入统一媒体库的电台会显示“已在媒体库”，避免重复操作。
