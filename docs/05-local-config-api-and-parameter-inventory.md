...
| `player.localPlayback` | HTTP/HTTPS 音频 URL、队列、网络电台及最长 60 分钟定时停止 | KPlayer UPnP AVTransport + 自有后台队列与内存定时器管理；电台写入自有配置 | 是，含传输状态、进度与倒计时 | **实验性：正常控制、手动/自动续播、电台持久化和定时停止已验收** | 是：异常格式、断网、AirPlay/蓝牙抢占和断电窗口仍需覆盖 | 否 | `GET /player`；`POST /player/control` |
